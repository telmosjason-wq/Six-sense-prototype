    ...
    <Modal title="RevvyAI" onClose={onClose} wide>
      ...
      {agentTab === "chat" && (
        <div style={{ display: "flex", flexDirection: "column", height: 480 }}>
          ...
          {messages.length <= 2 && <div style={{ display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>{quickActions.map(q => <button key={q} onClick={() => { setInput(q); setTimeout(() => { const fakeMsg = q; setMessages(m => [...m, { role: "user", text: fakeMsg }]); setInput(""); setTyping(true); if (useLLM) { const hist = [...messages.filter(m => m.role !== "system"), { role: "user", text: fakeMsg }]; callClaude(hist, sysPrompt.current).then(resp => { setMessages(m => [...m, { role: "assistant", text: resp || cannedRespond(fakeMsg, accounts, contacts) }]); setTyping(false); }); } else { setTimeout(() => { setMessages(m => [...m, { role: "assistant", text: cannedRespond(fakeMsg, accounts, contacts) }]); setTyping(false); }, 600); } }, 50); }} style={{ padding: "4px 10px", borderRadius: 12, border: `1px solid ${C.border}`, background: "transparent", color: C.textMuted, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>{q}</button>)}</div>}
